...
function isExpenseRoot(category: Category) {
  const name = getCategoryName(category).toLowerCase()
  const kind = getCategoryKind(category)

  return kind === 'expense' || kind === 'expenses' || name === 'wydatki'
}
...
function buildCategoryOptions(
  ...
  const root = categories.find(isExpenseRoot)
...
export default function ExpenseCategoryTrendWidget({
  rect,
  transactions,
  selectedMonth,
  budgetStartDate,
  ...
  const data: MonthPoint[] = useMemo(() => {
    ...
    transactions.forEach((transaction) => {
      ...
      selectedOptions.forEach((option) => {
        ...
        monthPoint.valuesBySeriesId[option.id] = (monthPoint.valuesBySeriesId[option.id] ?? 0) + Math.abs(amount)
      })
    })
  ...
  if (options.length === 0) {
    return <div style={emptyStyle}>Brak danych wydatkowych dla kategorii.</div>
  }
